...
OUTPUT_DIR="${2:-"$ROOT_DIR/docs/playtest-captures/keyboard-fallback-latest"}"
SMOKE_ARG="--rift-road-keyboard-fallback-smoke"
OUTPUT_ARG_PREFIX="--rift-road-keyboard-fallback-output="
CAPTURE_ARG_PREFIX="--rift-road-keyboard-fallback-capture-dir="
OUTPUT_JSON=""
CAPTURE_PATH=""
...
wait_for_artifacts() {
  ...
  echo "ERROR: Exported app did not write keyboard fallback artifacts: $OUTPUT_JSON $CAPTURE_PATH" >&2
  exit 1
}
...
if missing:
    print(f"ERROR: Keyboard fallback smoke checks failed: {', '.join(missing)}", file=sys.stderr)
    sys.exit(1)
if payload.get("capture") != str(capture_path):
    print(f"ERROR: Keyboard fallback capture path mismatch: {payload.get('capture')} != {capture_path}", file=sys.stderr)
    sys.exit(1)
if payload.get("automated") is not True:
    print("ERROR: Keyboard fallback payload must be labeled automated", file=sys.stderr)
    sys.exit(1)
PY
