...
rm -f "$TITLE_CAPTURE" "$HERO_SELECT_CAPTURE" "$GAMEPLAY_CAPTURE" "$COMBAT_CAPTURE" "$ROAD_COLLAPSE_CAPTURE" "$STAGE_CLEAR_CAPTURE" "$GAME_OVER_CAPTURE" "$RETRY_GAMEPLAY_CAPTURE"
...
wait_for_captures() {
  for _attempt in {1..80}; do
    if [[ -s "$TITLE_CAPTURE" && -s "$HERO_SELECT_CAPTURE" && -s "$GAMEPLAY_CAPTURE" && -s "$COMBAT_CAPTURE" && -s "$ROAD_COLLAPSE_CAPTURE" && -s "$STAGE_CLEAR_CAPTURE" && -s "$GAME_OVER_CAPTURE" && -s "$RETRY_GAMEPLAY_CAPTURE" ]]; then
      return 0
    fi
    sleep 0.5
  done

  echo "ERROR: Exported app did not write smoke captures: $TITLE_CAPTURE $HERO_SELECT_CAPTURE $GAMEPLAY_CAPTURE $COMBAT_CAPTURE $ROAD_COLLAPSE_CAPTURE $STAGE_CLEAR_CAPTURE $GAME_OVER_CAPTURE $RETRY_GAMEPLAY_CAPTURE" >&2
  exit 1
}
...
echo "RIFT_ROAD_EXPORTED_APP_SMOKE ok title_capture=$TITLE_CAPTURE hero_select_capture=$HERO_SELECT_CAPTURE gameplay_capture=$GAMEPLAY_CAPTURE combat_capture=$COMBAT_CAPTURE road_collapse_capture=$ROAD_COLLAPSE_CAPTURE stage_clear_capture=$STAGE_CLEAR_CAPTURE game_over_capture=$GAME_OVER_CAPTURE retry_gameplay_capture=$RETRY_GAMEPLAY_CAPTURE"
