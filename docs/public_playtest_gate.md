...
- Every counted row includes a 64-character `package_sha256=<sha>` value and signed `package_source=...Rift Road-signed-notarized.zip` in `Build`, plus a real non-empty `Evidence capture` file.
