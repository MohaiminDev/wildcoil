...
- [ ] Use `bash scripts/collect_playtest_evidence.sh --help` after each external-style session to generate a session note and paste-ready log row; inside a signed known-tester packet, the collector records the selected signed artifact by default. Keep the generated `package_sha256=<sha>` and signed `package_source=...Rift Road-signed-notarized.zip` build metadata.
- [ ] After adding session rows to `docs/playtest_log.md`, every row has `package_sha256=<sha>` and signed package source metadata in `Build`, every row has a real non-empty `Evidence capture` file, and `bash scripts/check_playtest_evidence.sh` reports `RIFT_ROAD_PLAYTEST_EVIDENCE public-playtest-candidate` before making any public-playtest-candidate claim.
...
- Every counted row includes `package_sha256=<sha>` and signed `package_source=...Rift Road-signed-notarized.zip` in `Build`, plus a real non-empty `Evidence capture` file.
