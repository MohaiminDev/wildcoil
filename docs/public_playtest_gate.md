...
- [ ] Use `bash scripts/collect_focus_audio_evidence.sh --help` during manual focus/audio sessions to generate evidence notes before updating `docs/focus_audio_validation.md`; inside a signed known-tester packet, the collector records the selected signed artifact by default. Keep the generated 64-character `package_sha256=<sha>` and signed `package_source=...Rift Road-signed-notarized.zip` build metadata.
- [ ] `bash scripts/check_focus_audio_evidence.sh` reports `RIFT_ROAD_FOCUS_AUDIO_EVIDENCE ok` after a human confirms audible output before focus loss, quiet/suspended audio while focus-paused, audible output after resume, package SHA metadata, and signed `package_source=...Rift Road-signed-notarized.zip` metadata in the session snippet.
- [ ] `bash scripts/check_second_machine_evidence.sh` reports `RIFT_ROAD_SECOND_MACHINE_EVIDENCE ok` before any public-playtest or release-candidate distribution claim, with `build/macos/Rift Road-signed-notarized.zip` and a 64-character `Package SHA256` digest recorded in the second-machine install smoke.
- [ ] Use `bash scripts/collect_controller_evidence.sh --help` during manual controller/keyboard sessions to generate evidence notes before updating `docs/controller_validation.md`; inside a signed known-tester packet, the collector records the selected signed artifact by default. Keep the generated 64-character `package_sha256=<sha>` and signed `package_source=...Rift Road-signed-notarized.zip` build metadata, and confirm special only after the luma/special meter is ready.
...
- [ ] Use `bash scripts/collect_playtest_evidence.sh --help` after each external-style session to generate a session note and paste-ready log row; inside a signed known-tester packet, the collector records the selected signed artifact by default. Keep the generated 64-character `package_sha256=<sha>` and signed `package_source=...Rift Road-signed-notarized.zip` build metadata.
- [ ] After adding session rows to `docs/playtest_log.md`, every row has a 64-character `package_sha256=<sha>` value and signed package source metadata in `Build`, every row has `machine=primary-mac` or `machine=second-mac` setup metadata, every row has a non-placeholder input method, every row has first-combat and wow-moment times inside the Phase 1 targets, every row has tester hook-description and show-someone-moment cells, every row has non-placeholder timing/replay/confusion/cheap-damage/quote/follow-up cells, no more than one counted row reports cheap damage, every row has a real non-empty `Evidence capture` file, and `bash scripts/check_playtest_evidence.sh` reports `RIFT_ROAD_PLAYTEST_EVIDENCE public-playtest-candidate` before making any public-playtest-candidate claim.
...
- Every counted row includes a 64-character `package_sha256=<sha>` value and signed `package_source=...Rift Road-signed-notarized.zip` in `Build`, `machine=primary-mac` or `machine=second-mac` setup metadata, a non-placeholder input method, first-combat and wow-moment times inside the Phase 1 targets, tester hook-description and show-someone-moment cells, non-placeholder timing/replay/confusion/cheap-damage/quote/follow-up cells, no repeated cheap-damage reports, plus a real non-empty `Evidence capture` file.
