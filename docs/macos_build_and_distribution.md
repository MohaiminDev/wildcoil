...
Also on 2026-05-10, `bash scripts/check_second_machine_evidence.sh` and `docs/second_machine_validation.md` were added to block release-candidate claims until a real second Apple Silicon Mac records host profile, install smoke, package SHA-256 metadata, Gatekeeper acceptance, and title/gameplay captures under `docs/playtest-captures/second-machine-latest/`. On 2026-05-12, `bash scripts/collect_second_machine_evidence.sh` was added to generate those files on the actual second Mac by reusing the package audit and exported-app smoke. The expected current result is `RIFT_ROAD_SECOND_MACHINE_EVIDENCE blocked`.
