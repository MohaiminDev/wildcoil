...
Also on 2026-05-12, the exported-app smoke, keyboard fallback smoke, and focus/resume smoke scripts moved to staged capture replacement: each run writes new artifacts under a temporary directory, validates them, and only then replaces the published `docs/playtest-captures/` evidence files. A launch failure now leaves the previous latest smoke evidence intact instead of deleting it before replacement proof exists.

Also on 2026-05-12, `bash scripts/check_focus_audio_evidence.sh` and `docs/focus_audio_validation.md` were added to block release-candidate claims until a human confirms audible output before focus loss, quiet/suspended audio while focus-paused, audible output after resume, and resume control from the exported macOS app. The gate now requires each marked focus/audio session to include a 64-character `package_sha256=<sha>` plus signed `package_source=...Rift Road-signed-notarized.zip` metadata in `Build` and a real non-empty evidence capture. `bash scripts/collect_focus_audio_evidence.sh` now generates a non-empty evidence note and paste-ready checklist snippet after a real manual focus/audio session, and it records the selected package path and package SHA metadata when available before falling back to `Rift Road.zip`; unsigned fallback snippets remain internal-only notes and do not satisfy the gate. The expected current result is `RIFT_ROAD_FOCUS_AUDIO_EVIDENCE blocked`.
...
Also on 2026-05-10, `bash scripts/prepare_known_tester_packet.sh` was added to create `build/known-tester-packet/latest/` with the rebuilt package, validation logs, package audit, exported-app smoke captures, exported-app keyboard fallback evidence, exported-app focus/resume evidence, exported-app performance JSON, host profile, and the current playtest protocol/docs. On 2026-05-11, the packet manifest began recording the build commit and package SHA-256 so tester notes can be tied to the exact zip under test. On 2026-05-12, the packet began bundling `scripts/collect_second_machine_evidence.sh` plus the audit/smoke/check helpers needed to collect second-machine proof on a real Apple Silicon Mac B, and it now records release signing status plus manual gate statuses and logs for focus/audio, playtest, controller, and second-machine evidence. The packet uses `Rift Road-signed-notarized.zip` when release signing succeeds and a signed artifact exists; otherwise it keeps using the internal `Rift Road.zip`. The manifest also surfaces the Godot version gate marker from `logs/check.log`, the selected package name, the selected package SHA-256, and the release signing status from `logs/release_signing.log`. A signed package in this packet is still not enough for public distribution until second-machine evidence, controller evidence, focus/audio evidence, and playtest gates pass.

Also on 2026-05-10, `bash scripts/check_playtest_evidence.sh` was added to read `docs/playtest_log.md` and block public-playtest-candidate status until the log records at least five external sessions, at least one `machine=second-mac` setup, at least two physical `controller-family=<family>` inputs, and majority replay intent. On 2026-05-12, the gate was hardened so every counted row must include a 64-character `package_sha256=<sha>`, a signed `package_source=...Rift Road-signed-notarized.zip` value in `Build`, `machine=primary-mac` or `machine=second-mac` setup metadata, a non-placeholder input method, first-combat and wow-moment times inside the Phase 1 targets, tester hook-description and show-someone-moment cells, non-placeholder timing/replay/confusion/cheap-damage/quote/follow-up cells, no repeated cheap-damage reports, and an `Evidence capture` path that resolves to a real, non-empty note, screenshot, or video file. The release-candidate gate now treats this as a blocker while the log has no external rows.

Also on 2026-05-12, `bash scripts/collect_playtest_evidence.sh` was added to generate non-empty session notes and paste-ready `docs/playtest_log.md` table rows after real external-style sessions. It requires an explicit `--confirm-external-session` flag plus the same timing, replay intent, confusion, cheap-damage, quote, and follow-up fields used by the playtest log. It records the selected package, a 64-character `package_sha256=<sha>`, and `package_source=<path>` metadata, but the evidence gate only counts rows whose build metadata names `Rift Road-signed-notarized.zip`; unsigned fallback rows remain internal-only notes and do not replace the playtest evidence gate.

Also on 2026-05-10, `bash scripts/check_second_machine_evidence.sh` and `docs/second_machine_validation.md` were added to block release-candidate claims until a real second Apple Silicon Mac records host profile, install smoke, signed/notarized package source, 64-character package SHA-256 metadata, Gatekeeper acceptance, and title/gameplay captures under `docs/playtest-captures/second-machine-latest/`. On 2026-05-12, `bash scripts/collect_second_machine_evidence.sh` was added to generate those files on the actual second Mac by reusing the package audit and exported-app smoke. The collector only writes `RIFT_ROAD_SECOND_MACHINE_INSTALL ok` when the host reports `arm64`, matching the evidence checker's Apple Silicon requirement. The expected current result is `RIFT_ROAD_SECOND_MACHINE_EVIDENCE blocked`.

Also on 2026-05-10, `bash scripts/check_controller_evidence.sh` and `docs/controller_validation.md` were added to block release-candidate claims until two real physical controller-family sessions plus keyboard fallback record title, hero select, Stage 1 movement, attack, jump, special-meter readiness, special, dash, pause, and cancel/back as `pass`. On 2026-05-11, the gate was hardened to reject marked sessions with missing or `TBD` build, device, connection, evidence-capture, or blocker metadata. On 2026-05-12, it was tightened again so each `Evidence capture` path must resolve to a real, non-empty file, each `Build` field must include a 64-character `package_sha256=<sha>` plus signed `package_source=...Rift Road-signed-notarized.zip` metadata, and future rows must confirm the luma/special meter was ready before marking the special action as passed. The expected current result is `RIFT_ROAD_CONTROLLER_EVIDENCE blocked`.

Also on 2026-05-12, `bash scripts/collect_controller_evidence.sh` was added to generate non-empty evidence notes and paste-ready controller-validation snippets after real exported-app controller or keyboard sessions. It requires explicit per-control confirmations, including special-meter readiness before special, records the selected package path plus package SHA by default, and still does not replace physical device testing; unsigned fallback snippets remain internal-only and do not satisfy the controller evidence gate.
...
Also on 2026-05-12, a launched exported-app keyboard menu pass exposed that text-style `j` input could reach title/hero menu paths but fail to start Stage 1 from the hero capability preview. `AppRoot` now normalizes menu key events through logical keycode, physical keycode, and selected printable unicode values before matching menu actions. `docs/playtest-captures/controller/20260512-keyboard-menu-confirm.md` records the rebuilt exported-app proof for title, hero select, capability-preview cancel/back, and `j` to Stage 1. This is not a full manual keyboard fallback row and does not replace the two required physical controller-family sessions.
