...
Also on 2026-05-10, `bash scripts/check_playtest_evidence.sh` was added to read `docs/playtest_log.md` and block public-playtest-candidate status until the log records at least five external sessions, at least one `machine=second-mac` setup, at least two physical `controller-family=<family>` inputs, and majority replay intent. On 2026-05-12, the gate was hardened so every counted row must include `package_sha256=<sha>`, a signed `package_source=...Rift Road-signed-notarized.zip` value in `Build`, and an `Evidence capture` path that resolves to a real, non-empty note, screenshot, or video file. The release-candidate gate now treats this as a blocker while the log has no external rows.

Also on 2026-05-12, `bash scripts/collect_playtest_evidence.sh` was added to generate non-empty session notes and paste-ready `docs/playtest_log.md` table rows after real external-style sessions. It requires an explicit `--confirm-external-session` flag plus the same timing, replay intent, confusion, cheap-damage, quote, and follow-up fields used by the playtest log. It records the selected package, `package_sha256=<sha>`, and `package_source=<path>` metadata, but the evidence gate only counts rows whose build metadata names `Rift Road-signed-notarized.zip`; unsigned fallback rows remain internal-only notes and do not replace the playtest evidence gate.
