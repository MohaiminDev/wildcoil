...
Also on 2026-05-10, `bash scripts/check_playtest_evidence.sh` was added to read `docs/playtest_log.md` and block public-playtest-candidate status until the log records at least five external sessions, at least one `machine=second-mac` setup, at least two physical `controller-family=<family>` inputs, and majority replay intent. On 2026-05-12, the gate was hardened so every counted row must include a 64-character `package_sha256=<sha>`, a signed `package_source=...Rift Road-signed-notarized.zip` value in `Build`, first-combat and wow-moment times inside the Phase 1 targets, non-placeholder timing/replay/confusion/cheap-damage/quote/follow-up cells, and an `Evidence capture` path that resolves to a real, non-empty note, screenshot, or video file. The release-candidate gate now treats this as a blocker while the log has no external rows.
