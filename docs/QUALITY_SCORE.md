...
|---|---:|---|---|---|---|---|
| Runtime launch | 4/5 | `tests/test_runtime_smoke.py`, `runtime_test_runner.gd`, `scripts/check.sh` | Version-specific Godot check | Supported Godot version | Local machine may have different Godot/export setup | Confirm and document supported Godot version |
...
| macOS packaging | 3/5 | `scripts/package_macos.sh`, `export_presets.cfg`, `docs/macos_build_and_distribution.md` | Clean-machine packaging test | Signing/notarization owner/process | Export templates and signing setup | Document exact Godot/export prerequisites |
