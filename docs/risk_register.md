...
- 2026-03-05 local Phase 1 smoke found first combat comfortably under the 30-second target and the exported macOS build stable, but the external-tester gate is still blocked by missing outside testers and no connected controller hardware.
...
| R-08 | Input | Controller behavior is unreliable on macOS | Core platform-fit requirement fails | Medium | High | Hot-plug issues, bad prompts, deadzone complaints | Test at least two controllers plus keyboard early | Solo developer | Input testing | Open - keyboard passed, controller hardware still missing |
...
| R-11 | Testing blind spots | Self-testing misses clarity, fairness, or pacing failures | Bad decisions survive too long | High | High | Feedback surprises after external playtests | Schedule 5 to 8 outside sessions for Phase 1 | Solo developer | Player testing | Blocked on outside testers |
