...
- Source-run local demo smoke: `bash scripts/smoke_source_run_local_demo.sh` reports `RIFT_ROAD_SOURCE_RUN_DEMO_SMOKE ok` and writes title, hero-select, opening story, gameplay, combat, pickups, road-collapse, Brask intro, Stage Clear, Game Over, and retry gameplay viewport captures under `docs/playtest-captures/source-run-demo-latest/`.
