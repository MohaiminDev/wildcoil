...
- Engine: Godot 4.6.1
- Validation: `bash scripts/check.sh` passed with 96 tests and Godot runtime smoke.
...
- Release-candidate gate: `bash scripts/check_release_candidate.sh` reports `RIFT_ROAD_RELEASE_GATE blocked` while package and player-evidence gates remain unresolved, and now includes the exported-app keyboard fallback smoke as an automated gate.
- Known-tester packet: `bash scripts/prepare_known_tester_packet.sh` creates `build/known-tester-packet/latest/` with the zip, manifest, build commit, package SHA-256, logs, docs, controller and second-machine checklists, smoke captures, keyboard fallback evidence, performance JSON, and host profile for supervised internal sessions.
