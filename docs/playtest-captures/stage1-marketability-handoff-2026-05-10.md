...
- Known-tester packet: `bash scripts/prepare_known_tester_packet.sh` creates `build/known-tester-packet/latest/` with the zip, manifest, logs, docs, controller and second-machine checklists, smoke captures, keyboard fallback evidence, performance JSON, and host profile for supervised internal sessions.
