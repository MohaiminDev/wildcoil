...
| `docs/design-docs/assets/kian-vale-male-concept.png` | Generated through the built-in image generation tool from project-specific Rift Road/Kian prompt guidance | Project-bound placeholder runtime art source | Approved separate Kian male concept for the current lead; runtime standee crops are saved under `src/wildcoil/assets/stage1/actors/kian_vale_*.png`. Replace or paint over during final sprite production. |
| `docs/design-docs/assets/tor-bram-male-concept.png` | Generated through the built-in image generation tool from project-specific Rift Road/Tor prompt guidance | Project-bound placeholder runtime art source | Approved separate Tor male concept for the second male roster slot; runtime standee crops are saved under `src/wildcoil/assets/stage1/actors/tor_bram_*.png`. Replace or paint over during final sprite production. |
| `docs/design-docs/assets/kian-tor-male-lineup-concept.png` | Generated through the built-in image generation tool from project-specific Rift Road male lineup prompt guidance | Project-bound placeholder concept reference | Supporting side-by-side male silhouette reference only; runtime uses the separate Kian and Tor images. |
...
| `src/wildcoil/assets/stage1/actors/kian_vale_*.png` | Runtime cutouts derived from `src/wildcoil/assets/stage1/source/kian_vale_runtime_alpha.png` | Project-bound placeholder runtime art | Image-backed Kian idle, walk, attack, and hurt slots for current source-run proof. These are clean transparent concept cutouts, not final animation sprites. |
| `src/wildcoil/assets/stage1/actors/tor_bram_*.png` | Runtime cutouts derived from `src/wildcoil/assets/stage1/source/tor_bram_runtime_alpha.png` | Project-bound placeholder runtime art | Image-backed Tor preview idle, walk, attack, and hurt slots. Tor remains preview-only until Kian's combat loop is proven. |
| `src/wildcoil/assets/stage1/source/kian_vale_runtime_chromakey.png`, `src/wildcoil/assets/stage1/source/kian_vale_runtime_alpha.png` | Generated through the built-in image generation tool with a flat chroma-key background, then processed locally with the installed chroma-key removal helper | Project-bound placeholder runtime art source | Clean transparent Kian runtime source used for the current `kian_vale_*.png` actor slots. |
| `src/wildcoil/assets/stage1/source/tor_bram_runtime_chromakey.png`, `src/wildcoil/assets/stage1/source/tor_bram_runtime_alpha.png` | Generated through the built-in image generation tool with a flat chroma-key background, then processed locally with the installed chroma-key removal helper | Project-bound placeholder runtime art source | Clean transparent Tor runtime source used for the current `tor_bram_*.png` actor slots. |
