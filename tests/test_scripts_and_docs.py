...
def test_playtest_evidence_gate_rejects_non_hex_package_sha(repo_root, tmp_path):
    script_path = repo_root / "scripts" / "check_playtest_evidence.sh"
    fake_log = tmp_path / "playtest_log.md"
    evidence_dir = tmp_path / "playtest-captures"
    evidence_dir.mkdir()
...
def test_playtest_evidence_gate_rejects_placeholder_session_observations(
    repo_root, tmp_path
):
    script_path = repo_root / "scripts" / "check_playtest_evidence.sh"
    fake_log = tmp_path / "playtest_log.md"
    ...
    assert result.returncode == 1
    assert "RIFT_ROAD_PLAYTEST_EVIDENCE blocked" in output
    assert "missing session observation: First-combat time" in output
    assert "missing session observation: Wow-moment time" in output
    assert "missing session observation: Replay desire" in output
    assert "missing session observation: Confusion points" in output
    assert "missing session observation: Cheap-damage reports" in output
    assert "missing session observation: Key quotes / observations" in output
    assert "missing session observation: Follow-up action" in output


def test_playtest_evidence_gate_rejects_slow_playtest_milestone_times(
    repo_root, tmp_path
):
    script_path = repo_root / "scripts" / "check_playtest_evidence.sh"
    fake_log = tmp_path / "playtest_log.md"
    ...
    def session_row(
        ...
        *,
        ...
        first_combat_time="00:24",
        wow_moment_time="02:10",
        replay_desire="yes",
    ):
        return (
            "2026-05-12",
    ...
    assert result.returncode == 1
    assert "RIFT_ROAD_PLAYTEST_EVIDENCE blocked" in output
    assert "First-combat time exceeds 30 seconds" in output
    assert "Wow-moment time exceeds 180 seconds" in output


def test_playtest_evidence_gate_rejects_missing_setup_and_input_metadata(
    repo_root, tmp_path
):
    script_path = repo_root / "scripts" / "check_playtest_evidence.sh"
    fake_log = tmp_path / "playtest_log.md"
    ...
    rows = [
        session_row(
            0,
            setup="macOS 26; Apple Silicon",
            input_method="keyboard; controller-family=xbox",
        ),
        session_row(1, setup="machine=second-mac", input_method="TBD"),
        session_row(2, input_method="keyboard; controller-family=dualshock"),
        session_row(3, replay_desire="no"),
        session_row(4),
    ]
    fake_log.write_text(
    ...
    assert result.returncode == 1
    assert "RIFT_ROAD_PLAYTEST_EVIDENCE blocked" in output
    assert "Setup missing machine=primary-mac or machine=second-mac" in output
...
def test_local_playtest_evidence_gate_blocks_until_real_source_run_session(
    repo_root, tmp_path
):
    script_path = repo_root / "scripts" / "check_local_playtest_evidence.sh"
    assert script_path.exists()
    ...
    assert "RIFT_ROAD_LOCAL_PLAYTEST_EVIDENCE blocked" in script
    assert "RIFT_ROAD_LOCAL_PLAYTEST_EVIDENCE source-run-session" in script
    assert "source_run=local" in script
    assert "RIFT_ROAD_LOCAL_PLAYABILITY_ok" in script
    assert "Rift Road-signed-notarized.zip" not in script
    assert "check_second_machine_evidence.sh" not in script
    assert "bash scripts/check_local_playtest_evidence.sh" in local_log

    for docs in [readme, tracker, audit, macos_docs]:
        assert "bash scripts/check_local_playtest_evidence.sh" in docs

    empty_result = subprocess.run(
        ["bash", str(script_path), str(repo_root / "docs" / "local_playtest_log.md")],
        cwd=repo_root,
        capture_output=True,
        text=True,
        check=False,
    ...
    empty_output = empty_result.stdout + empty_result.stderr
    assert "RIFT_ROAD_LOCAL_PLAYTEST_EVIDENCE blocked" in empty_output
    assert "completed sessions 0/1" in empty_output

    evidence_path = tmp_path / "local-session.md"
    evidence_path.write_text("human local source-run notes\n")
    fake_log = tmp_path / "local_playtest_log.md"
    fake_log.write_text(
        "\n".join(
            [
                "# Local Log",
                "",
                "| Date | Build | Evidence capture | Tester | Input method | Route result | First-combat time | Wow-moment time | Session result | Replay desire | Felt good | Show-someone moment | Confusion points | Cheap-damage reports | Follow-up action |",
                "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
                (
                    f"| 2026-05-13 | commit=abc123 source_run=local "
                    f"local_playability=RIFT_ROAD_LOCAL_PLAYABILITY_ok | {evidence_path} | "
                    "local-dev | keyboard | Stage Clear | 00:24 | 02:10 | "
                    "cleared Stage 1 and returned to title | yes - would replay | "
                    "movement and opening fight stayed readable | road collapse | "
                    "none | none | record longer human video |"
                ),
                "",
            ]
        )
    ...
    assert ok_result.returncode == 0, ok_result.stdout + ok_result.stderr
    assert "RIFT_ROAD_LOCAL_PLAYTEST_EVIDENCE source-run-session" in ok_result.stdout
    assert "sessions=1/1" in ok_result.stdout
    assert "evidence_captures=1/1" in ok_result.stdout
...
def test_second_machine_evidence_gate_rejects_non_hex_package_sha(
    repo_root, tmp_path
):
    script_path = repo_root / "scripts" / "check_second_machine_evidence.sh"
    evidence_dir = tmp_path / "second-machine-latest"
    ...
    assert result.returncode == 1
    assert "RIFT_ROAD_SECOND_MACHINE_EVIDENCE blocked" in output
    assert "install smoke does not record a 64-character Package SHA256" in output
