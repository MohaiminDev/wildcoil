...
- Focus/audio evidence gate: `bash scripts/check_focus_audio_evidence.sh` reads `docs/focus_audio_validation.md` and currently reports `RIFT_ROAD_FOCUS_AUDIO_EVIDENCE blocked` because no manual audible focus-loss/resume session has been recorded. The gate now rejects marked sessions unless their `Build` metadata includes `package_sha256=<sha>` plus signed `package_source=...Rift Road-signed-notarized.zip` metadata and their `Evidence capture` path points at a real, non-empty file. `bash scripts/collect_focus_audio_evidence.sh --help` now generates manual evidence notes and paste-ready session snippets for real exported-app focus/audio sessions, records the selected package path and package SHA by default, but unsigned fallback rows remain internal-only and it does not turn the gate green without actual human-confirmed audible output.
