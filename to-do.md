...
The current operating target is source-run local playability on this computer. Developer ID signing/notarization, second-machine/source-install proof, and clean-machine Gatekeeper evidence remain downloadable app distribution gates, but they are not a source-run blocker for proving the code is playable locally end to end through `bash scripts/run_game.sh` and `bash scripts/check_local_playability.sh`.
...
The playable path is: title screen, hero select, Stage 1 opening Kian/Nika story panels, Kian-led combat against a reference Iron Veil grunt, readable later enemy waves, simple pickups, road-collapse set piece into a lower service-lane beat, Brask Noll boss with readable heavy-attack/stun lesson, Brask escape and route-to-Glassleaf consequence, Stage Clear score/rank summary, restart or return-to-title flow, and packaged macOS zip.
...
- Hero-select demo scope: Kian is the playable Stage 1 local-demo lead for RR-PROD-109. Tor, Raya, and Nika remain visible as roster preview cards until the Kian-vs-Iron-Veil-grunt reference loop feels strong enough to copy.
...
- Package audit: `bash scripts/audit_macos_package.sh` reports `RIFT_ROAD_PACKAGE_AUDIT internal-only`; the bundle signature verifies, but Developer ID authority, Apple Team ID, notarization, Gatekeeper acceptance, and stapled ticket validation are not present. This is a downloadable app distribution concern, not a source-run blocker.
...
- Signing preflight/release script: `bash scripts/check_macos_signing_env.sh` reports `RIFT_ROAD_SIGNING_PREFLIGHT blocked`; required non-secret inputs are `RIFT_ROAD_APPLE_TEAM_ID`, `RIFT_ROAD_DEVELOPER_ID_APPLICATION`, and `RIFT_ROAD_NOTARY_KEYCHAIN_PROFILE`. `docs/macos_release_inputs.example.env` now provides a redacted local setup template; real values must stay outside version control. `bash scripts/sign_notarize_macos.sh` now defines the guarded `build/macos/Rift Road-signed-notarized.zip` path, but currently reports `RIFT_ROAD_RELEASE_SIGNING blocked` until real local Apple signing/notary inputs exist. This remains necessary for downloadable app distribution, but it is not a source-run blocker.
- Second-machine evidence: `bash scripts/check_second_machine_evidence.sh` reads `docs/playtest-captures/second-machine-latest/` and currently reports `RIFT_ROAD_SECOND_MACHINE_EVIDENCE blocked` because no clean-machine proof files have been recorded. The gate now rejects install-smoke proof without 64-character `Package SHA256` metadata or signed/notarized package source. `bash scripts/collect_second_machine_evidence.sh` is now bundled for known testers so the real second Mac can generate the expected host profile, package audit log, install-smoke note, package SHA metadata, and title/gameplay captures; it only writes `RIFT_ROAD_SECOND_MACHINE_INSTALL ok` when the host reports `arm64`. Unsigned `Rift Road.zip` fallback remains internal-only handoff evidence, while release evidence must name `Rift Road-signed-notarized.zip`. This remains necessary for downloadable app distribution, but it is not a source-run blocker.
...
- Focus/audio evidence gate: `bash scripts/check_focus_audio_evidence.sh` reads `docs/focus_audio_validation.md` and currently reports `RIFT_ROAD_FOCUS_AUDIO_EVIDENCE blocked` because no manual audible focus-loss/resume session has been recorded. The gate now rejects marked sessions unless their `Build` metadata includes a 64-character `package_sha256=<sha>` plus signed `package_source=...Rift Road-signed-notarized.zip` metadata and their `Evidence capture` path points at a real, non-empty file. `bash scripts/collect_focus_audio_evidence.sh --help` now generates manual evidence notes and paste-ready session snippets for real exported-app focus/audio sessions, records the selected package path and package SHA by default, but unsigned fallback rows remain internal-only and it does not turn the gate green without actual human-confirmed audible output.
...
- Playtest evidence gate: `bash scripts/check_playtest_evidence.sh` reads `docs/playtest_log.md` and currently reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` because no external session rows have been recorded. The gate now rejects counted rows unless their `Build` metadata includes a 64-character `package_sha256=<sha>` plus signed `package_source=...Rift Road-signed-notarized.zip` metadata, their `Setup` metadata includes `machine=primary-mac` or `machine=second-mac`, their `Input method` is not a placeholder, their first-combat and wow-moment times are inside the Phase 1 targets, their hook-description and show-someone-moment cells are present, their timing/replay/confusion/cheap-damage/quote/follow-up cells are not placeholders, repeated cheap-damage reports are absent, and their `Evidence capture` path points at a real, non-empty file.
- Playtest evidence collector: `bash scripts/collect_playtest_evidence.sh --help` now generates a non-empty session note and paste-ready `docs/playtest_log.md` row after a real external-style session, records the selected package path and package SHA by default, but unsigned fallback rows remain internal-only and it does not append rows or satisfy the gate without actual tester evidence.
- 2026-05-11 spec/story realignment: [`docs/game_spec.md`](/Users/himu/Desktop/career/personal_projects/wildcoil/docs/game_spec.md) and [`docs/game-story.md`](/Users/himu/Desktop/career/personal_projects/wildcoil/docs/game-story.md) now make the first success condition feel-focused, not market-demand-focused: the Stage 1 slice must feel good, look alive, and be satisfying to replay on an M1 iMac before public-playtest or marketability claims. Current missing broader proof still includes physical controller validation and external playtest evidence; second-machine/source-install proof is out of scope for source-run local playability.
...
- Progress:
  - 2026-05-13: Added approved separate male concept references for Kian Vale and Tor Bram, generated project-local runtime standee crops, wired Tor into the visual asset manifest and hero-select image path, and refreshed source-run smoke screenshots showing both male heroes in hero select.
...
### [RR-PROD-108] Keep planned heroes preview-only in local demo
- Outcome: Kian and Tor remain visible in hero select as future roster previews, but only Raya and Nika can start Stage 1 in the current local demo.
- Validation:
  - [x] Added a failing runtime regression requiring Tor preview to stay preview-only while Nika still starts Stage 1.
  - [x] Added a playable-hero gate in `src/wildcoil/scripts/app_root.gd` and updated hero-select/preview prompts to distinguish playable-now heroes from planned preview cards.
  - [x] Added a failing runtime regression requiring the exported/source smoke capture path to ignore focus-loss pause while the normal focus-loss pause/resume test still passes.
  - [x] Hardened `scripts/smoke_source_run_local_demo.sh` with capture-dir environment fallback coverage, source-state provenance, phase diagnostics, window activation, and a bounded watchdog so source-run evidence cannot hang silently.
  - [x] Updated product/tracker docs to record the current playable demo scope.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_source_run_local_demo_smoke_records_viewport_evidence tests/test_runtime_smoke.py::test_character_select_preview_blocks_planned_heroes tests/test_runtime_smoke.py::test_exported_smoke_capture_ignores_focus_loss_pause tests/test_runtime_smoke.py::test_stage_one_focus_loss_pauses_and_resumes tests/test_runtime_smoke.py::test_stage_one_title_to_victory_flow tests/test_runtime_smoke.py::test_keyboard_fallback_title_to_stage_and_action_flow tests/test_runtime_smoke.py::test_keyboard_text_confirm_event_starts_stage_one -q` passes with 7 tests.
  ...
  - [x] `bash scripts/check.sh` passes with 142 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-13: Tightened hero select so the local demo path cannot drift into unfinished Kian/Tor slots, then hardened the source-run smoke after validation exposed a focus/background capture stall.
- Dependencies: [RR-PROD-107]
...
- Validation:
  - [x] Added a failing regression requiring the local playtest gate to block the placeholder log and accept one complete source-run row with a real evidence file.
  - [x] Added `bash scripts/check_local_playtest_evidence.sh` and documented it in the local log, README, macOS notes, market-readiness audit, and tracker.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_local_playtest_evidence_gate_blocks_until_real_source_run_session -q` passes.
  - [x] `bash -n scripts/check_local_playtest_evidence.sh` passes.
  - [x] `bash scripts/check_local_playtest_evidence.sh` currently reports `RIFT_ROAD_LOCAL_PLAYTEST_EVIDENCE blocked` for the real remaining local human-session row gap.
  - [x] `bash scripts/check_local_playability.sh` reports `RIFT_ROAD_LOCAL_PLAYABILITY ok scope=source-run machine=local`.
  - [x] `python3 scripts/check_agent_docs.py` passes.
  - [x] `git diff --check` passes.
  - [x] `bash scripts/check.sh` passes with 140 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-13: Added a mechanical verifier for future local human source-run session notes.
- Dependencies: [RR-PROD-105]
- Completed: 2026-05-13
...
- Validation:
  - [x] Added a failing regression requiring the local playability gate, docs references, and explicit separation from downloadable app distribution blockers.
  - [x] Added `bash scripts/check_local_playability.sh` as the source-run local playability command.
  - [x] Updated README, macOS distribution notes, market-readiness audit, and tracker scope so Developer ID signing/notarization and second-machine/source-install proof are not source-run blockers.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_local_source_playability_gate_is_documented_and_distribution_free -q` passes.
  - [x] `bash -n scripts/check_local_playability.sh` passes.
  - [x] `bash scripts/check_local_playability.sh` reports `RIFT_ROAD_LOCAL_PLAYABILITY ok scope=source-run machine=local`.
  - [x] `python3 scripts/check_agent_docs.py` passes.
  - [x] `git diff --check` passes.
  - [x] `bash scripts/check.sh` passes with 138 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-13: Separated local source-run playability from downloadable app distribution evidence.
- Dependencies: [RR-PROD-103]
- Completed: 2026-05-13

### [RR-PROD-103] Require playtest hook and show-moment proof
- Outcome: `scripts/check_playtest_evidence.sh` now rejects legacy or incomplete external-session rows that omit hook-description and show-someone-moment cells, so public-playtest-candidate proof must show whether testers can describe Rift Road's hook and identify a share-worthy moment.
- Validation:
  - [x] Added a failing regression proving otherwise valid legacy external-session rows are rejected when hook-description and show-someone-moment cells are missing.
  - [x] Updated the playtest evidence checker to align legacy rows as missing the new fields and require hook/show-moment observations for every counted row.
  - [x] Updated the playtest evidence collector to generate hook-description and show-someone-moment cells and notes.
  - [x] Updated playtest log, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming external tester sessions exist.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_playtest_evidence_gate_requires_hook_and_show_moment_fields tests/test_scripts_and_docs.py::test_playtest_evidence_collector_scaffolds_external_session_notes tests/test_scripts_and_docs.py::test_manual_evidence_collectors_default_to_signed_packet_artifact tests/test_scripts_and_docs.py::test_playtest_evidence_gate_requires_real_evidence_capture_files tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_repeated_cheap_damage_reports tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_missing_setup_and_input_metadata tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_slow_playtest_milestone_times tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_placeholder_session_observations tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_unsigned_package_source -q`
  - [x] `bash -n scripts/check_playtest_evidence.sh`
  - [x] `bash -n scripts/collect_playtest_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_playtest_evidence.sh` still reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` for the real remaining external-session gaps.
  - [x] `bash scripts/check.sh` passes with 137 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened public-playtest evidence rows against missing hook and show-someone proof before real external playtest evidence exists.
- Dependencies: [RR-PROD-102]
- Completed: 2026-05-12

### [RR-PROD-102] Block repeated cheap-damage playtest proof
- Outcome: `scripts/check_playtest_evidence.sh` now rejects public-playtest-candidate proof when more than one counted external-session row reports cheap damage, so repeated unfair-hit patterns cannot pass as market-ready evidence.
- Validation:
  - [x] Added a failing regression proving otherwise valid external-session rows with two cheap-damage reports are rejected.
  - [x] Added a repeated cheap-damage counter and blocker to the playtest evidence checker.
  - [x] Updated playtest log, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming external tester sessions exist.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_repeated_cheap_damage_reports tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_missing_setup_and_input_metadata tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_slow_playtest_milestone_times tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_placeholder_session_observations tests/test_scripts_and_docs.py::test_playtest_evidence_gate_requires_real_evidence_capture_files tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_unsigned_package_source -q`
  - [x] `bash -n scripts/check_playtest_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_playtest_evidence.sh` still reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` for the real remaining external-session gaps.
  - [x] `bash scripts/check.sh` passes with 136 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened public-playtest evidence rows against repeated cheap-damage reports before real external playtest proof exists.
- Dependencies: [RR-PROD-101]
- Completed: 2026-05-12

### [RR-PROD-101] Require playtest setup and input metadata
- Outcome: `scripts/check_playtest_evidence.sh` now rejects counted external-session rows that omit `machine=primary-mac` or `machine=second-mac` setup metadata, or leave `Input method` as `TBD`, so public-playtest-candidate proof cannot pass from aggregate second-Mac/controller counts while individual rows lack setup/input identity.
- Validation:
  - [x] Added a failing regression proving otherwise valid external-session rows are rejected when one lacks `machine=` setup metadata and another leaves `Input method` as `TBD`.
  - [x] Added row-level setup and input-method blockers to the playtest evidence checker.
  - [x] Updated playtest log, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming external tester sessions exist.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_missing_setup_and_input_metadata tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_slow_playtest_milestone_times tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_placeholder_session_observations tests/test_scripts_and_docs.py::test_playtest_evidence_gate_requires_real_evidence_capture_files tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_unsigned_package_source -q`
  - [x] `bash -n scripts/check_playtest_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_playtest_evidence.sh` still reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` for the real remaining external-session gaps.
  - [x] `bash scripts/check.sh` passes with 135 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened public-playtest evidence rows against missing setup/input metadata before real external playtest proof exists.
- Dependencies: [RR-PROD-100]
- Completed: 2026-05-12

### [RR-PROD-100] Enforce playtest milestone timing targets
- Outcome: `scripts/check_playtest_evidence.sh` now rejects counted external-session rows whose first-combat time exceeds 30 seconds or whose wow-moment time exceeds 180 seconds, so public-playtest-candidate proof must satisfy the Phase 1 timing targets instead of merely filling the time cells.
- Validation:
  - [x] Added a failing regression proving otherwise valid external-session rows with first combat at `00:31` or wow moment at `03:01` are rejected.
  - [x] Added milestone timestamp parsing and threshold blockers to the playtest evidence checker.
  - [x] Updated playtest log, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming external tester sessions exist.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_slow_playtest_milestone_times tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_placeholder_session_observations tests/test_scripts_and_docs.py::test_playtest_evidence_gate_requires_real_evidence_capture_files tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_unsigned_package_source -q`
  - [x] `bash -n scripts/check_playtest_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_playtest_evidence.sh` still reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` for the real remaining external-session gaps.
  - [x] `bash scripts/check.sh` passes with 134 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened public-playtest evidence rows against slow milestone timings before real external playtest proof exists.
- Dependencies: [RR-PROD-99]
- Completed: 2026-05-12

### [RR-PROD-99] Require real playtest observation fields
- Outcome: `scripts/check_playtest_evidence.sh` now rejects counted external-session rows that leave timing, replay, confusion, cheap-damage, quote/observation, or follow-up cells as `TBD`, so public-playtest-candidate proof cannot pass from package metadata plus empty session observations.
- Validation:
  - [x] Added a failing regression proving five otherwise valid external-session rows with signed package metadata, real evidence files, second-Mac coverage, two controller families, and replay intent are rejected when session observation cells contain `TBD`.
  - [x] Updated the playtest evidence checker to require non-placeholder timing/replay/confusion/cheap-damage/quote/follow-up cells for every counted row.
  - [x] Updated playtest log, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming external tester sessions exist.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_placeholder_session_observations tests/test_scripts_and_docs.py::test_playtest_evidence_gate_requires_real_evidence_capture_files tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_unsigned_package_source -q`
  - [x] `bash -n scripts/check_playtest_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_playtest_evidence.sh` still reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` for the real remaining external-session gaps.
  - [x] `bash scripts/check.sh` passes with 133 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened public-playtest evidence rows against placeholder session observations before real external playtest proof exists.
- Dependencies: [RR-PROD-98]
- Completed: 2026-05-12

### [RR-PROD-98] Require real SHA-256 shape in playtest evidence
- Outcome: `scripts/check_playtest_evidence.sh` now rejects counted external-session rows whose `Build` metadata contains `package_sha256=` but not a 64-character hex digest, so placeholder package identity values cannot satisfy public-playtest-candidate evidence.
- Validation:
  - [x] Added a failing regression proving five otherwise valid external-session rows with `package_sha256=fake`, signed package sources, second-Mac coverage, two controller families, replay intent, and real evidence files are rejected.
  - [x] Updated the playtest evidence checker to preserve the missing-SHA blocker while adding an invalid-SHA blocker.
  - [x] Updated the positive evidence-capture fixture to use a 64-character package SHA value.
  - [x] Updated playtest log, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming external tester sessions exist.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_requires_real_evidence_capture_files tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_unsigned_package_source -q`
  - [x] `bash -n scripts/check_playtest_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_playtest_evidence.sh` still reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` for the real remaining external-session gaps.
  - [x] `bash scripts/check.sh` passes with 132 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened external playtest package identity metadata before real public-playtest proof is available.
- Dependencies: [RR-PROD-89]
- Completed: 2026-05-12

### [RR-PROD-97] Require real SHA-256 shape in controller evidence
- Outcome: `scripts/check_controller_evidence.sh` now rejects marked physical controller and manual keyboard fallback sessions whose `Build` metadata contains `package_sha256=` but not a 64-character hex digest, so placeholder package identity values cannot satisfy controller release evidence.
- Validation:
  - [x] Added a failing regression proving two complete controller-family rows plus one keyboard fallback row with placeholder `package_sha256` values, signed package sources, full control checks, no blockers, and real evidence files are rejected.
  - [x] Updated the shared controller/keyboard build metadata validator to preserve the missing-SHA blocker while adding an invalid-SHA blocker.
  - [x] Updated the positive complete-session fixture to use 64-character package SHA values.
  - [x] Updated controller validation, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming physical controller evidence exists.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_controller_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_controller_evidence_gate_accepts_complete_session_metadata tests/test_scripts_and_docs.py::test_controller_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_controller_evidence_gate_rejects_unsigned_package_source -q`
  - [x] `bash -n scripts/check_controller_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_controller_evidence.sh` still reports `RIFT_ROAD_CONTROLLER_EVIDENCE blocked` for the real remaining physical-controller and manual keyboard fallback gaps.
  - [x] `bash scripts/check.sh` passes with 131 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened manual controller/keyboard package identity metadata before real device proof is available.
- Dependencies: [RR-PROD-90]
- Completed: 2026-05-12

### [RR-PROD-96] Require real SHA-256 shape in focus/audio evidence
- Outcome: `scripts/check_focus_audio_evidence.sh` now rejects marked manual focus/audio sessions whose `Build` metadata contains `package_sha256=` but not a 64-character hex digest, so placeholder package identity values cannot satisfy audible-output release evidence.
- Validation:
  - [x] Added a failing regression proving a complete-looking manual focus/audio session with `package_sha256=fake`, signed package source, all focus/audio checks, no blockers, and a real evidence file is rejected.
  - [x] Updated `scripts/check_focus_audio_evidence.sh` to preserve the missing-SHA blocker while adding an invalid-SHA blocker.
  - [x] Updated the focus/audio collector regression to use a 64-character package SHA in the generated accepted snippet.
  - [x] Updated focus/audio validation, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming audible output evidence exists.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_focus_audio_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_focus_audio_evidence_collector_scaffolds_manual_audible_session tests/test_scripts_and_docs.py::test_focus_audio_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_focus_audio_evidence_gate_rejects_unsigned_package_source -q`
  - [x] `bash -n scripts/check_focus_audio_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_focus_audio_evidence.sh` still reports `RIFT_ROAD_FOCUS_AUDIO_EVIDENCE blocked` for the real remaining manual audible-session gap.
  - [x] `bash scripts/check.sh` passes with 130 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened manual focus/audio package identity metadata before real audible-output proof is available.
- Dependencies: [RR-PROD-91]
- Completed: 2026-05-12

### [RR-PROD-95] Require real SHA-256 shape in second-machine evidence
- Outcome: `scripts/check_second_machine_evidence.sh` now rejects clean-machine install-smoke proof whose `Package SHA256` metadata is present but not a 64-character hex digest, so hand-written placeholder values such as `signed123` cannot satisfy second-machine release evidence.
- Validation:
  - [x] Added a failing regression proving a complete-looking second-machine install-smoke note with `Package SHA256: signed123`, signed package source, release-candidate package status, Gatekeeper acceptance, install-ok marker, and captures is rejected.
  - [x] Updated `scripts/check_second_machine_evidence.sh` to preserve the missing-SHA blocker while adding a 64-character digest-shape blocker.
  - [x] Updated the positive signed-package second-machine fixture to use a 64-character hex digest.
  - [x] Updated second-machine, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming second-machine evidence exists.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_accepts_signed_package_source tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_rejects_missing_package_sha -q`
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_rejects_missing_package_sha tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_rejects_non_hex_package_sha tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_rejects_unsigned_package_source tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_accepts_signed_package_source tests/test_scripts_and_docs.py::test_second_machine_collector_blocks_non_arm64_hosts -q`
  - [x] `bash -n scripts/check_second_machine_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_second_machine_evidence.sh` still reports `RIFT_ROAD_SECOND_MACHINE_EVIDENCE blocked` for the real remaining clean-machine proof gap.
  - [x] `bash scripts/check.sh` passes with 129 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened second-machine package identity metadata before real clean-machine proof is available.
- Dependencies: [RR-PROD-94]
- Completed: 2026-05-12

### [RR-PROD-94] Block second-machine collector ok on non-arm64 hosts
- Outcome: `scripts/collect_second_machine_evidence.sh` no longer writes `RIFT_ROAD_SECOND_MACHINE_INSTALL ok` or exits successfully when the labeled second-machine run is on a non-`arm64` host, keeping the collector aligned with the evidence gate's Apple Silicon requirement.
- Validation:
  - [x] Added a failing regression that fakes a signed package, release-candidate audit, Gatekeeper acceptance, and launched-app captures on an `x86_64` host, then requires the collector to stay blocked.
  - [x] Updated the second-machine collector's install-ok predicate to require `arm64`.
  - [x] Updated second-machine, macOS distribution, market-readiness, handoff, and tracker docs without claiming second-machine evidence exists.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_second_machine_collector_blocks_non_arm64_hosts -q`
  ...
  - [x] `bash -n scripts/collect_second_machine_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_second_machine_evidence.sh` still reports `RIFT_ROAD_SECOND_MACHINE_EVIDENCE blocked` for the real remaining clean-machine proof gap.
  - [x] `bash scripts/check.sh` passes with 128 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Prevented a false collector-ok marker before real second Apple Silicon Mac proof is available.
- Dependencies: [RR-PROD-61]
- Completed: 2026-05-12
...
- Validation:
  - [x] `bash scripts/check_release_candidate.sh` ran outside the sandbox, passed the 126-test `bash scripts/check.sh` phase, refreshed launched exported-app smoke, keyboard fallback, focus/resume, exported-app performance, and headless performance evidence, and wrote `build/release-gate/latest/completion-audit.md`.
  - [x] Release gate correctly reported `RIFT_ROAD_RELEASE_GATE blocked` for signing/notarization, focus/audio, playtest, second-machine, controller, public-playtest, and player-love evidence gaps.
  ...
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check.sh` passes with 126 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Rebased tracked release-gate evidence on the current focus/audio signed-source hardening branch without claiming public-playtest or release-candidate readiness.
- Dependencies: [RR-PROD-91]
- Completed: 2026-05-12

### [RR-PROD-91] Require signed package source in focus/audio evidence
- Outcome: The focus/audio evidence gate now rejects marked manual audible-output sessions whose `Build` metadata names the unsigned internal `Rift Road.zip` package source, so release-candidate focus/audio proof must name the signed/notarized zip used during the manual audible-output session.
- Validation:
  - [x] Added a failing regression proving a complete focus/audio session with a real evidence file, package SHA metadata, and unsigned package source is rejected.
  - [x] Updated `scripts/check_focus_audio_evidence.sh` to require signed `package_source=...Rift Road-signed-notarized.zip` metadata for marked focus/audio rows.
  - [x] Updated `scripts/collect_focus_audio_evidence.sh` to write selected package-source metadata by default and warn that custom build metadata must keep the signed package source.
  - [x] Updated focus/audio validation, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming audible output evidence exists.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_focus_audio_evidence_gate_rejects_unsigned_package_source tests/test_scripts_and_docs.py::test_focus_audio_evidence_collector_scaffolds_manual_audible_session tests/test_scripts_and_docs.py::test_focus_audio_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_focus_audio_evidence_gate_blocks_without_manual_audible_confirmation tests/test_scripts_and_docs.py::test_manual_evidence_collectors_default_to_signed_packet_artifact -q`
  - [x] `bash -n scripts/check_focus_audio_evidence.sh`
  - [x] `bash -n scripts/collect_focus_audio_evidence.sh`
  - [x] `bash scripts/check_focus_audio_evidence.sh` still reports `RIFT_ROAD_FOCUS_AUDIO_EVIDENCE blocked` for the real remaining manual audible-output gap.
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check.sh` passes with 126 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened manual focus/audio evidence so future audible-output rows cannot use the internal unsigned package path for a release-candidate claim.
- Dependencies: [RR-PROD-85]
- Completed: 2026-05-12

### [RR-PROD-90] Require signed package source in controller evidence
- Outcome: The controller evidence gate now rejects marked controller and keyboard fallback rows whose `Build` metadata names the unsigned internal `Rift Road.zip` package source, so release-candidate controller proof must name the signed/notarized zip used during each manual device session.
- Validation:
  - [x] Added a failing regression proving two complete controller-family rows plus one keyboard fallback row with real evidence files, package SHA metadata, special-meter-ready confirmation, and unsigned package source are rejected.
  - [x] Updated `scripts/check_controller_evidence.sh` to require signed `package_source=...Rift Road-signed-notarized.zip` metadata for controller and keyboard fallback rows.
  - [x] Updated `scripts/collect_controller_evidence.sh` to write selected package-source metadata by default and warn that custom build metadata must keep the signed package source.
  - [x] Updated controller validation, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming physical controller sessions exist.
  ...
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check.sh` passes with 125 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened release-candidate controller evidence so future physical-device rows cannot use the internal unsigned package path for a candidate claim.
- Dependencies: [RR-PROD-83]
- Completed: 2026-05-12

### [RR-PROD-89] Require signed package source in playtest evidence
- Outcome: The playtest evidence gate now rejects counted external-session rows whose `Build` metadata names the unsigned internal `Rift Road.zip` package source, so `public-playtest-candidate` proof must name the signed/notarized zip used by each tester session.
- Validation:
  - [x] Added a failing regression proving a five-session playtest log with real evidence notes, second-Mac coverage, two controller families, replay intent, package SHA metadata, and unsigned package source is rejected.
  - [x] Updated `scripts/check_playtest_evidence.sh` to require signed `package_source=...Rift Road-signed-notarized.zip` metadata for counted rows.
  - [x] Updated `scripts/collect_playtest_evidence.sh` to write selected package-source metadata by default and warn that custom build metadata must keep the signed package source.
  - [x] Updated playtest log, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming external tester sessions exist.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_unsigned_package_source tests/test_scripts_and_docs.py::test_playtest_evidence_gate_requires_real_evidence_capture_files tests/test_scripts_and_docs.py::test_playtest_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_playtest_evidence_collector_scaffolds_external_session_notes tests/test_scripts_and_docs.py::test_manual_evidence_collectors_default_to_signed_packet_artifact -q`
  - [x] `bash -n scripts/check_playtest_evidence.sh`
  - [x] `bash -n scripts/collect_playtest_evidence.sh`
  ...
  - [x] `git diff --check`
  - [x] `bash scripts/check_playtest_evidence.sh` still reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` for the real remaining external-session gaps.
  - [x] `bash scripts/check.sh` passes with 124 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened public-playtest evidence so future external-session rows cannot use the internal unsigned package path for a candidate claim.
- Dependencies: [RR-PROD-84]
- Completed: 2026-05-12

### [RR-PROD-88] Refresh release-gate snapshot after evidence hardening
- Outcome: The release-candidate gate was rerun after the package-SHA and signed-source evidence hardening, refreshing tracked launched-app smoke, keyboard fallback, focus/resume, and exported-app performance evidence while preserving the real release blockers.
- Validation:
  - [x] `bash scripts/check_release_candidate.sh` ran outside the sandbox, passed the 123-test `bash scripts/check.sh` phase, refreshed launched exported-app smoke, keyboard fallback, focus/resume, exported-app performance, and headless performance evidence, and wrote `build/release-gate/latest/completion-audit.md`.
  - [x] Release gate correctly reported `RIFT_ROAD_RELEASE_GATE blocked` for signing/notarization, focus/audio, playtest, second-machine, controller, public-playtest, and player-love evidence gaps.
  - [x] Updated `docs/performance_budget.md`, `docs/market-readiness-audit-2026-05-10.md`, `docs/playtest-captures/stage1-marketability-handoff-2026-05-10.md`, and this tracker with the refreshed 1280x720 exported-app performance values.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_exported_app_performance_docs_match_latest_json -q`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check.sh` passes with 123 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Rebased tracked release-gate evidence on the current evidence-hardening branch without claiming public-playtest or release-candidate readiness.
- Dependencies: [RR-PROD-87]
- Completed: 2026-05-12

### [RR-PROD-87] Require signed package source in second-machine evidence
- Outcome: The second-machine evidence gate now rejects clean-machine install-smoke proof that names the unsigned internal `Rift Road.zip` package source, so public-playtest or release-candidate second-Mac evidence must name the signed/notarized zip.
- Validation:
  - [x] Added a failing regression proving a complete-looking second-machine install-smoke file with unsigned package source, package SHA, release-candidate status, Gatekeeper acceptance, install-ok marker, and captures is rejected.
  - [x] Updated `scripts/check_second_machine_evidence.sh` to require `build/macos/Rift Road-signed-notarized.zip` in the install-smoke package source.
  - [x] Updated second-machine, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming second-machine evidence exists.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_rejects_unsigned_package_source tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_accepts_signed_package_source tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_rejects_missing_package_sha tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_blocks_without_clean_machine_proof -q`
  - [x] `bash -n scripts/check_second_machine_evidence.sh`
  - [x] `bash -n scripts/collect_second_machine_evidence.sh`
  ...
  - [x] `bash scripts/check.sh` passes with 123 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened second-machine release evidence so future Apple Silicon Mac B install proof cannot use the internal unsigned package path.
- Dependencies: [RR-PROD-14]
- Completed: 2026-05-12

### [RR-PROD-86] Require package SHA in second-machine evidence
- Outcome: The second-machine evidence gate now rejects clean-machine install-smoke proof that omits `Package SHA256`, so second-Mac release evidence must identify the exact zip installed and launched.
- Validation:
  - [x] Added a failing regression proving a complete-looking second-machine install-smoke file with release-candidate package status, Gatekeeper acceptance, install-ok marker, and captures is rejected when it omits `Package SHA256`.
  - [x] Updated `scripts/check_second_machine_evidence.sh` to reject missing or placeholder package SHA metadata.
  - [x] Updated the positive signed-package second-machine fixture to include package SHA metadata.
  - [x] Updated second-machine, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming second-machine evidence exists.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_rejects_missing_package_sha tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_accepts_signed_package_source tests/test_scripts_and_docs.py::test_second_machine_evidence_gate_blocks_without_clean_machine_proof tests/test_scripts_and_docs.py::test_second_machine_evidence_collector_is_bundled_for_known_testers -q`
  - [x] `bash -n scripts/check_second_machine_evidence.sh`
  - [x] `bash -n scripts/collect_second_machine_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_second_machine_evidence.sh` still reports `RIFT_ROAD_SECOND_MACHINE_EVIDENCE blocked` for the real remaining clean-machine proof gap.
  - [x] `bash scripts/check.sh` passes with 122 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened second-machine evidence so future Apple Silicon Mac B install proof must identify the exact package artifact.
- Dependencies: [RR-PROD-14]
- Completed: 2026-05-12

### [RR-PROD-85] Require package SHA in focus/audio evidence
- Outcome: The focus/audio evidence gate now rejects marked manual audible-output sessions whose `Build` metadata does not include `package_sha256=<sha>`, so release-candidate focus/audio proof must name the exact packaged artifact under test.
- Validation:
  - [x] Added a failing regression proving a complete focus/audio session with a real evidence note is rejected when `Build` omits `package_sha256=<sha>`.
  - [x] Updated `scripts/check_focus_audio_evidence.sh` to reject marked sessions missing package SHA metadata.
  - [x] Verified the existing collector default already writes `commit=<short> package_sha256=<sha>` and updated its help/docs to warn that custom build values must keep `package_sha256=<sha>`.
  - [x] Updated focus/audio, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming audible focus/audio evidence exists.
  - [x] `python3 -m pytest tests/test_scripts_and_docs.py::test_focus_audio_evidence_gate_rejects_build_metadata_without_package_sha tests/test_scripts_and_docs.py::test_focus_audio_evidence_gate_blocks_without_manual_audible_confirmation tests/test_scripts_and_docs.py::test_focus_audio_evidence_collector_scaffolds_manual_audible_session tests/test_scripts_and_docs.py::test_manual_evidence_collectors_default_to_signed_packet_artifact -q`
  - [x] `bash -n scripts/check_focus_audio_evidence.sh`
  - [x] `bash -n scripts/collect_focus_audio_evidence.sh`
  - [x] `python3 scripts/check_agent_docs.py`
  - [x] `git diff --check`
  - [x] `bash scripts/check_focus_audio_evidence.sh` still reports `RIFT_ROAD_FOCUS_AUDIO_EVIDENCE blocked` for the real remaining manual audible-session gap.
  - [x] `bash scripts/check.sh` passes with 121 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened manual focus/audio evidence so future audible-output rows must identify the exact package artifact.
- Dependencies: [RR-PROD-14]
- Completed: 2026-05-12

### [RR-PROD-84] Require package SHA in playtest evidence
- Outcome: The playtest evidence gate now rejects counted external-session rows whose `Build` metadata does not include `package_sha256=<sha>`, so public-playtest-candidate proof must name the exact packaged artifact used by each tester session instead of a loose commit string.
- Validation:
  - [x] Added a failing regression proving a five-session playtest log with real evidence notes, second-Mac coverage, two controller families, and replay intent is rejected when `Build` omits `package_sha256=<sha>`.
  - [x] Updated `scripts/check_playtest_evidence.sh` to reject counted rows missing package SHA metadata.
  - [x] Verified the existing collector default already writes `commit=<short> package_sha256=<sha>` and updated its help/docs to warn that custom build values must keep `package_sha256=<sha>`.
  - [x] Updated playtest, public gate, macOS distribution, market-readiness, handoff, and tracker docs without claiming external tester sessions exist.
  ...
  - [x] `git diff --check`
  - [x] `bash scripts/check_playtest_evidence.sh` still reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` for the real remaining external-session gaps.
  - [x] `bash scripts/check.sh` passes with 120 tests and `RIFT_ROAD_RUNTIME_OK smoke`.
- Progress:
  - 2026-05-12: Hardened RR-PROD-14 evidence so future external playtest rows must identify the exact package artifact.
- Dependencies: [RR-PROD-14]
- Completed: 2026-05-12

### [RR-PROD-83] Require package SHA in controller evidence
- Outcome: The controller/keyboard evidence gate now rejects marked manual rows whose `Build` metadata does not include `package_sha256=<sha>`, so controller and keyboard fallback proof must be tied to the exact packaged artifact under test instead of a loose commit string.
- Validation:
  - [x] Added a failing regression proving two complete controller-family rows plus one keyboard fallback row are rejected when their `Build` fields omit `package_sha256=<sha>`.
  - [x] Updated `scripts/check_controller_evidence.sh` to reject controller and keyboard fallback rows missing package SHA metadata.
  - [x] Verified the existing collector default already writes `commit=<short> package_sha256=<sha>` and updated its help/docs to warn that custom build values must keep `package_sha256=<sha>`.
