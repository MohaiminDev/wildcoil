...
- Playtest evidence gate: `bash scripts/check_playtest_evidence.sh` reads `docs/playtest_log.md` and currently reports `RIFT_ROAD_PLAYTEST_EVIDENCE blocked` because no external session rows have been recorded. The gate now rejects counted rows unless their `Build` metadata includes `package_sha256=<sha>` plus signed `package_source=...Rift Road-signed-notarized.zip` metadata, and their `Evidence capture` path points at a real, non-empty file.
